...
function AdminProductdetail() {
  ...
  const handleCart = (e) => {
    ...
    if (items.some((item) => item.product.id === product.id)) {
      const itemQuantity = items.filter((item) => item.product.id === product.id)[0].quantity;
      dispatch(UpdateCartAsync({ product: product.id, quantity: itemQuantity + 1, user: User.id }));
    } else {
        ...
        <div className="pt-6">
          ...
          <div className="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
            ...
            <div className="aspect-h-5 aspect-w-4 lg:aspect-h-4 lg:aspect-w-3 sm:overflow-hidden sm:rounded-lg">
              <img src={product.images[3] || product.images[0]} alt={product.title} className="h-full w-full object-cover object-center" />
            </div>
          </div>
